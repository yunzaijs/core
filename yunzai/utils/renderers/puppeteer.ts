...
export default class Puppeteer extends Renderer {
  ...
  constructor(config?: PuppeteerLaunchOptions & {
    ...
    this.config.executablePath = config?.chromiumPath ?? config.executablePath ?? cfg?.bot?.chromium_path
  ...
  async browserInit() {
    ...
    if (!this.browser || !connectFlag) {
      ...
      // 如果没有实例，初始化puppeteer
      this.browser = await puppeteer.launch(this.config).catch(Error as any) as any
    }
