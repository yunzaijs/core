...
class DailyCache extends BaseModel$1 {
    ...
    static async clearOutdatedData() {
        ...
        const date = moment().format('MM-DD');
        const testReg = new RegExp(`^${BOT_ROOT_KEY}(mys|hoyolab|config)-\\d{2}-\\d{2}`);
...
class MysApi {
    ...
    get device() {
        if (!this._device)
            this._device = `Yz-${md5(this.uid).substring(0, 5)}`;
        return this._device;
    }
    getUrl(type, data = {}) {
    ...
    async getData(type, data = {}, cached = false) {
        ...
        if (type === 'getFp' && !data?.Getfp)
            return this._device_fp;
    ...
    async getAgent() {
        ...
        if (!proxyAddress)
            return null;
        if (proxyAddress === 'http://0.0.0.0:0')
            return null;
        if (!/os_|official/.test(this.server))
            return null;
...
class MysUser extends BaseModel$1 {
    ...
    static async getByQueryUid(uid, game = 'gs', onlySelfCk = false) {
        ...
        if (ret) {
            logger.mark(`[米游社查询][uid：${uid}]${logger.chalk.green(`[使用已查询ck：${ret.ltuid}]`)}`);
            return ret;
        }
        if (onlySelfCk)
            return false;
        ...
        if (ret) {
            logger.mark(`[米游社查询][uid：${uid}]${logger.chalk.green(`[分配查询ck：${ret.ltuid}]`)}`);
            return ret;
        }
        return false;
    }
...
export { apiTool$1 as ApiTool, BOT_MYS_GENSHIN, BOT_NOTE_USER, BOT_ROOT_KEY, BaseModel$1 as BaseModel, DailyCache, GSCfg, MysApi, MysInfo, MysUser, MysUserDB, MysUtil, NoteUser, UserDB, UserGameDB, apiTool, createDir, forEach, getRoot, gsCfg, isPromise, mysApi, mysInfo, sequelize };
