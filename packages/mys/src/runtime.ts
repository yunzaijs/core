...
import { BOT_NAME, ConfigController as CFG, Handler, puppeteer } from 'yunzaijs'
import NoteUser from './NoteUser.js'
import MysUser from './MysUser.js'
import gsCfg from './gsCfg.js'
import { EventEmun } from 'yunzaijs'
import MysApi from './mysApi.js'
import MysInfo from './mysInfo.js'

type MessageType = Parameters<EventEmun['message']>[0]
...
export class Runtime {
  ...
  async render(pluginName, basePath, data: any = {}, cfg: any = {}) {
    ...
    // 自动计算pluResPath
    const pluResPath = `../../../${repeat(
      '../',
      paths.length
    )}plugins/${pluginName}/resources/`
    const miaoResPath = `../../../${repeat(
      '../',
      paths.length
    )}plugins/${PName}/resources/`
    ...
    data = {
      ...
      /** miao 相关参数 **/
      copyright: `Created By ${BOT_NAME}<span class="version">${
